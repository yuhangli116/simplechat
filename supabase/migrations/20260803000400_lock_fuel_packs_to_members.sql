...
DECLARE
  ...
  v_membership_expired BOOLEAN := FALSE;
BEGIN
  ...
  IF p_billing_group_id IS NOT NULL AND p_billing_step IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      v_membership_expired := v_log_expires_at IS NOT NULL AND v_log_expires_at < TIMEZONE('utc'::text, NOW());
      ...
      RETURN json_build_object(
        ...
        'member_diamonds_remaining', CASE WHEN v_membership_expired THEN 0 ELSE COALESCE(v_member_diamonds, 0) END,
        'permanent_diamonds_remaining', CASE WHEN v_membership_expired THEN 0 ELSE COALESCE(v_permanent_diamonds, 0) END,
        'total_remaining', CASE WHEN v_membership_expired THEN 0 ELSE COALESCE(v_member_diamonds, 0) + COALESCE(v_permanent_diamonds, 0) END
      );
    END IF;
  END IF;
  ...
  IF v_membership_expired AND v_diamonds_needed > 0 THEN
    RETURN json_build_object(
      'success', FALSE,
      'error', '会员已到期，请先续费后再使用加油包钻石',
